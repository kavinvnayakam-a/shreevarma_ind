...
export default function ProfileClient() {
    ...
    const consultationsQuery = useMemo(() => {
        if (!firestore || !user) return null;
        return query(collection(firestore, `consultations`), where('userId', '==', user.uid));
    }, [firestore, user]);
                                    ...
                                    {areAppointmentsLoading ? (
                                        ...
                                        <ul className="space-y-4">
                                            {sortedAppointments.map((appt) => {
                                                const isJoinable = appt.status === 'Upcoming' && appt.doctor?.name !== 'Unassigned';
                                                ...
                                                <li key={appt.__docId} className="flex flex-col sm:flex-row items-start sm:items-center sm:justify-between p-4 bg-muted/50 rounded-lg gap-4 border">
                                                    ...
                                                    <div className="flex items-center gap-2 sm:gap-4 self-end sm:self-center">
                                                        ...
                                                        <Button
                                                            ...
                                                            >
                                                            {isJoinable && !isEnded ? (
                                                                <Link href={`/consultation/session/${appt.__docId}`}>
                                                                ...
                                                                <button>
                                                                    <Video className="mr-2 h-4 w-4" />
                                                                    {isEnded ? 'Consultation Ended' : 'Assigning Doctor...'}
                                                                </button>
                                                            )}
                                                        </Button>
                                                    </div>
